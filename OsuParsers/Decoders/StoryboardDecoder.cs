...
public static class StoryboardDecoder
{
    ...
    private static void ParseSbObject(string line)
    {
        ...
        if (!Enum.TryParse(tokens[0], out EventType type))
            return;

        string sampleEventToken = tokens[type == EventType.Sample ? 2 : 1];
        var layer = (StoryboardLayer)Enum.Parse(typeof(StoryboardLayer), sampleEventToken);
            ...
            default:
            {
                ...
                switch (type)
                {
                    ...
                    case "V":
                    {
                        ...
                        float endX = tokens.Length > 6 ? ParseHelper.ToFloat(tokens[6]) : startX;
                        float endY = tokens.Length > 7 ? ParseHelper.ToFloat(tokens[7]) : startY;
                        commandGroup.Commands.Add(new Command(CommandType.VectorScale, easing, startTime, endTime, new Vector2(startX, startY), new Vector2(endX, endY)));
                        break;
                    }
                    case "R":
                    {
                    ...
                    case "C":
                    {
                        ...
                        commandGroup.Commands.Add(new Command(easing, startTime, endTime, Color.FromArgb(255, (int)startRed, (int)startGreen, (int)startBlue), Color.FromArgb(255, (int)endRed, (int)endGreen, (int)endBlue)));
                        break;
                    }
                    case "P":
                    {
                        ...
                        {
                            case "H":
                                commandType = CommandType.FlipHorizontal;
                                break;
                            case "V":
                                commandType = CommandType.FlipVertical;
                                break;
                            case "A":
                                commandType = CommandType.BlendingMode;
                                break;
                        }

                        if (commandType is not CommandType.None)
                        {
                            var command = new Command(commandType, easing, startTime, endTime);
