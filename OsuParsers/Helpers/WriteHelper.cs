...
using CurveTypeEnum = Enums.Beatmaps.CurveType;
...
internal class WriteHelper
{
    ...
    public static string HitObject(HitObject hitObject)
    {
        ...
        // TODO: Change again to not rely on the concrete Slider type
        if (hitObject is ICircleHitObject)
            extra += extras;
        if (hitObject is Slider slider)
            extra += SliderProperties(slider) + ((slider.EdgeHitSounds == null || !slider.EdgeHitSounds.Any()) ? string.Empty : $",{extras}");
        if (hitObject is ISpinnerHitObject spinner)
            extra += $"{hitObject.EndTime},{extras}";
    ...
    public static char CurveType(CurveType value)
    {
        return value switch
        {
            CurveTypeEnum.Bezier => 'B',
            CurveTypeEnum.Catmull => 'C',
            CurveTypeEnum.Linear => 'L',
            CurveTypeEnum.PerfectCurve => 'P',
    ...
    public static int TypeByte(HitObject hitObject)
    {
        ...
        if (hitObject is ISpinnerHitObject)
    ...
    public static List<string> StoryboardObject(IStoryboardObject storyboardObject, StoryboardLayer layer)
    {
        ...
        switch (storyboardObject)
        {
            ...
            {
                string line = $"""
                    Sprite,{layer},{sprite.Origin},"{sprite.FilePath}",{sprite.X.Format()},{sprite.Y.Format()}
                    """;
                list.Add(line);
                break;
            }
            ...
            {
                string line = $"""
                    Animation,{layer},{animation.Origin},"{animation.FilePath}",{animation.X.Format()},{animation.Y.Format()},{animation.FrameCount},{animation.FrameDelay},{animation.LoopType}
                    """;
                list.Add(line);
                break;
            }
            case StoryboardSample sample:
            {
                string line = $"""
                    Sample,{sample.Time},{layer},"{sample.FilePath}",{sample.Volume}
                    """;
                list.Add(line);
                break;
            }
